...
const makeFallbackStyle = (useLocalVectorTiles: boolean): maplibregl.StyleSpecification => ({
  ...
  layers: useLocalVectorTiles ? [
    ...
    { id: 'landcover', type: 'fill', source: 'russia', 'source-layer': 'landcover', paint: { 'fill-color': '#e5ecd8', 'fill-opacity': 0.82 } },
    { id: 'landuse', type: 'fill', source: 'russia', 'source-layer': 'landuse', paint: { 'fill-color': '#dce8c7', 'fill-opacity': 0.68 } },
    { id: 'parks', type: 'fill', source: 'russia', 'source-layer': 'park', paint: { 'fill-color': '#cde5ba', 'fill-opacity': 0.82 } },
    { id: 'water', type: 'fill', source: 'russia', 'source-layer': 'water', paint: { 'fill-color': '#a9d3e8', 'fill-opacity': 0.92 } },
    { id: 'boundary', type: 'line', source: 'russia', 'source-layer': 'boundary', paint: { 'line-color': '#9ca3af', 'line-width': ['interpolate', ['linear'], ['zoom'], 5, 0.5, 12, 1.1], 'line-dasharray': [2, 2] } },
    { id: 'roads-minor', type: 'line', source: 'russia', 'source-layer': 'transportation', filter: ['match', ['get', 'class'], ['minor', 'service', 'residential', 'unclassified', 'track', 'path'], true, false], paint: { 'line-color': '#ffffff', 'line-width': ['interpolate', ['linear'], ['zoom'], 10, 0.35, 13, 0.9, 17, 2.1], 'line-opacity': 0.95 } },
    { id: 'roads-tertiary', type: 'line', source: 'russia', 'source-layer': 'transportation', filter: ['match', ['get', 'class'], ['tertiary', 'secondary'], true, false], paint: { 'line-color': '#f8f4ea', 'line-width': ['interpolate', ['linear'], ['zoom'], 7, 0.7, 12, 1.5, 17, 3.6], 'line-opacity': 0.98 } },
    { id: 'roads-primary', type: 'line', source: 'russia', 'source-layer': 'transportation', filter: ['match', ['get', 'class'], ['primary', 'trunk', 'motorway'], true, false], paint: { 'line-color': '#f5c878', 'line-width': ['interpolate', ['linear'], ['zoom'], 5, 0.9, 10, 1.8, 14, 3.8, 17, 6.5], 'line-opacity': 0.98 } },
    { id: 'railway', type: 'line', source: 'russia', 'source-layer': 'transportation', filter: ['==', ['get', 'class'], 'rail'], paint: { 'line-color': '#7f8792', 'line-width': ['interpolate', ['linear'], ['zoom'], 8, 0.6, 14, 1.6], 'line-dasharray': [2, 1] } },
    { id: 'roads-name', type: 'symbol', source: 'russia', 'source-layer': 'transportation_name', minzoom: 11, layout: { 'text-field': ['coalesce', ['get', 'name:ru'], ['get', 'name_ru'], ['get', 'name'], ['get', 'name:latin'], ['get', 'ref']], 'text-font': ['Open Sans Regular'], 'text-size': ['interpolate', ['linear'], ['zoom'], 11, 10, 15, 12], 'symbol-placement': 'line', 'text-max-angle': 35, 'text-padding': 3 }, paint: { 'text-color': '#4b5563', 'text-halo-color': '#ffffff', 'text-halo-width': 1.35 } },
    { id: 'buildings', type: 'fill', source: 'russia', 'source-layer': 'building', minzoom: 13, paint: { 'fill-color': '#dfcdb6', 'fill-opacity': 0.65, 'fill-outline-color': '#d1b899' } },
    { id: 'water-name', type: 'symbol', source: 'russia', 'source-layer': 'water_name', layout: { 'text-field': ['coalesce', ['get', 'name:ru'], ['get', 'name_ru'], ['get', 'name'], ['get', 'name:latin']], 'text-font': ['Open Sans Regular'], 'text-size': 11, 'symbol-placement': 'line' }, paint: { 'text-color': '#4c7899', 'text-halo-color': '#ffffff', 'text-halo-width': 1 } },
    { id: 'place-labels', type: 'symbol', source: 'russia', 'source-layer': 'place', layout: { 'text-field': ['coalesce', ['get', 'name:ru'], ['get', 'name_ru'], ['get', 'name'], ['get', 'name:latin']], 'text-font': ['Open Sans Regular'], 'text-size': ['interpolate', ['linear'], ['zoom'], 5, 11, 12, 16], 'text-max-width': 8 }, paint: { 'text-color': '#303030', 'text-halo-color': '#ffffff', 'text-halo-width': 1.4 } },
