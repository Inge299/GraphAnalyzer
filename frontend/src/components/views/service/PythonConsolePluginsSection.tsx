...
import type { ConsoleProfile } from '../../../types/api';

interface PluginDraft extends ConsoleProfile {
  draftActive: boolean;
...
interface PythonConsolePluginsSectionProps {
  onMessage: (message: string | null) => void;
  onError: (message: string | null) => void;
}

const toDraft = (plugin: ConsoleProfile): PluginDraft => ({
  ...plugin,
...
const sourceName = (plugin: ConsoleProfile) => {
...
const PythonConsolePluginsSection: React.FC<PythonConsolePluginsSectionProps> = ({
  onMessage,
  onError,
}) => {
  ...
  const loadPlugins = useCallback(async () => {
    ...
    onError(null);
    try {
      const response = await consoleApi.pythonPlugins();
      setPlugins((response.plugins || []).map(toDraft));
    } catch (error: any) {
      onError(String(error?.response?.data?.detail || error?.message || 'Не удалось загрузить аналитические Python-плагины'));
    } finally {
      setLoading(false);
    }
  }, [onError]);
  ...
  const handleInstall = async (file: File) => {
    ...
    onError(null);
    try {
      const result = await consoleApi.installPythonPlugin(file);
      await loadPlugins();
      const names = result.plugins.map((plugin) => plugin.name).join(', ');
      onMessage(names ? `Подключены аналитические плагины: ${names}.` : `Подключён модуль ${result.filename}.`);
    } catch (error: any) {
      onError(String(error?.response?.data?.detail || error?.message || 'Не удалось подключить аналитический Python-плагин'));
    } finally {
      setManaging(false);
    }
  };
  ...
  const handleDelete = async (plugin: PluginDraft) => {
    ...
    onError(null);
    try {
      const result = await consoleApi.deletePythonPlugin(plugin.id);
      await loadPlugins();
      onMessage(`Внешний модуль ${result.filename} удалён.`);
    } catch (error: any) {
      onError(String(error?.response?.data?.detail || error?.message || 'Не удалось удалить аналитический Python-плагин'));
    } finally {
      setManaging(false);
    }
  };
  ...
  const savePlugin = async (plugin: PluginDraft) => {
    ...
    onError(null);
    try {
      const updated = await consoleApi.updatePythonPlugin(plugin.id, {
      ...
      onError(String(error?.response?.data?.detail || error?.message || 'Не удалось сохранить настройки аналитического плагина'));
    } finally {
      setSavingId(null);
    }
  };
  ...
  return (
    <div className="service-card service-python-plugins-card">
      <div className="service-card-header">
        <div>
          <h3>Аналитические Python-плагины</h3>
      ...
      <div className="service-row">
        ...
        />
        <label className={`service-btn primary ${managing ? 'disabled' : ''}`} htmlFor="console-python-plugin-file">
      ...
      {plugins.length === 0 && !loading ? (
        <div className="service-empty">Аналитические Python-плагины не обнаружены.</div>
      ) : (
...
export default PythonConsolePluginsSection;