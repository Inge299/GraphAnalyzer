...
Подробный пошаговый порядок для изолированного сервера приведён в
[`closed-contour-install.md`](closed-contour-install.md).
...
конфигурации, `INSTALL.md` и `SHA256SUMS.txt`. Передайте весь этот каталог, а также нужные файлы
