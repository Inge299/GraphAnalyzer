...
MIN_WIDTH = 1.5
MAX_WIDTH = 10.0
COUNT_KEYS = ("calls_count", "facts_count", "connections_count", "events_count", "contacts_count")
...
class EdgeWeightStylerPlugin(PluginBase):
    ...
    async def execute(self, input_artifacts: List[Dict[str, Any]], params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        weighted = [(edge, _connection_count(edge)) for edge in edges if isinstance(edge, dict)]
        ...
        for edge, count in weighted:
            if count is None or count <= 0:
                continue
            width = self._width_for(count, max_count)
            attributes = edge.get("attributes") if isinstance(edge.get("attributes"), dict) else {}
    ...
    @staticmethod
    def _width_for(count: int, max_count: int) -> float:
        if count <= 1 or max_count <= 1:
            return MIN_WIDTH
        ratio = math.log(count) / math.log(max_count)
