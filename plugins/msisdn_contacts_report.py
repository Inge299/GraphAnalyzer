...
class MsisdnContactsReportPlugin(PluginBase):
    ...
    def _period_share(contact: Dict[str, Any], event_times: List[datetime]) -> str:
        """Return the part of the MSISDN activity interval occupied by the contact.

        A communication can precede the first or follow the last known registration.
        Only the intersection with the analyzed MSISDN activity period is relevant;
        otherwise the former calculation could report more than 100 percent.
        """
        if not event_times:
            return "-"
        ...
        activity_start = min(event_times)
        activity_end = max(event_times)
        if last < first:
            return "-"
        overlap_start = max(first, activity_start)
        overlap_end = min(last, activity_end)
        if overlap_end < overlap_start:
            return "0.0%"

        activity_seconds = (activity_end - activity_start).total_seconds()
        if activity_seconds <= 0:
            return "100.0%" if overlap_start == activity_start and overlap_end == activity_end else "0.0%"
        overlap_seconds = (overlap_end - overlap_start).total_seconds()
        share = min(100.0, max(0.0, overlap_seconds * 100 / activity_seconds))
        return f"{share:.1f}%"
